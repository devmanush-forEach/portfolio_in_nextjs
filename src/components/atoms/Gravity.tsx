...
const Gravity: React.FC = () => {
  ...
  useEffect(() => {
    Physics({ timestep: 8, sleepDisabled: true }, function (world) {
      ...
      renderer = Physics.renderer("canvas", {
        el: physicsContainerRef.current,
      });
      ...
      for (var row = 0, l = 35; row < l; ++row) {
        for (var col = 0, lcol = 35; col < lcol; ++col) {
          ...
          if (col > 0) {
            // horizontal
            rigidConstraints.distanceConstraint(
              cloth[lcol * row + col - 1],
              cloth[lcol * row + col],
              0.4
            );
          }
      ...
      world.on(
        ...
        function () {
          var constraints =
              rigidConstraints.getConstraints().distanceConstraints,
            c,
            threshold = 60,
          ...
          for (var i = 0, l = constraints.length; i < l; ++i) {
            ...
            if (
              c.bodyA.treatment !== "static" &&
              c.bodyB.treatment !== "static" &&
              len - c.targetLength > threshold
            ) {
      ...
      world.on("render", function (data) {
        ...
        for (var i = 0, l = constraints.length; i < l; ++i) {
          c = constraints[i];
          ctx.moveTo(
            c.bodyA.state.pos.x + c.bodyA.state.vel.x * t,
            c.bodyA.state.pos.y + c.bodyA.state.vel.y * t
          );
          ctx.lineTo(
            c.bodyB.state.pos.x + c.bodyB.state.vel.x * t,
            c.bodyB.state.pos.y + c.bodyB.state.vel.y * t
          );
        }
        ctx.stroke();
      });
  ...
  return (
    <div ref={physicsContainerRef} style={{ width: "100%", height: "100vh" }} />
  );
};
